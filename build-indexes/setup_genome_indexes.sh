...
function make_fastq_screen_conf() {
    # Make a conf file for fastq_screen
    local screen_name=$1
    local organisms=$2
    local description=$3
    local index_type=$4
    ...
    if [ -z "$index_type" ] || [ "$index_type" == "color" ] ; then
	# Color space indexes
	index_type="colorspace"
	local bowtie_ext=_c
	local conf_ext=
    else
	# Nucleotide space indexes
    ...
    # Conf file
    local conf_file=fastq_screen/fastq_screen_${screen_name}${conf_ext}.conf
...
EOF
	...
	# Find bowtie index files
	local bowtie_base=`pwd`/${organism}/bowtie/${organism}${bowtie_ext}
	if [ -f "${bowtie_base}.1.ebwt" ] ; then
	    # Add to screen file
	    echo "" >> $conf_file
	    echo "## $organism ##" >> $conf_file
	    echo "DATABASE"$'\t'${organism}$'\t'${bowtie_base} >> $conf_file
	else
...
# Nucleotide space
make_fastq_screen_conf model_organisms \
    "$FASTQ_SCREEN_MODEL_ORGANISMS" "Model organisms" nt
make_fastq_screen_conf other_organisms \
    "$FASTQ_SCREEN_OTHER_ORGANISMS" "Other organisms" nt
#
###########################################################
...
if [ -f rRNAs.tar.gz ] ; then
    ...
    log=`mktemp --suffix=_rRNA_screen.log`
    ${SCRIPT_DIR}/build_rRNA_bowtie_indexes.sh rRNAs.tar.gz > $log 2>&1
    if [ $? -ne 0 ] ; then
	echo FAILED
	echo See log: $log
    else
    ...
    echo "No rRNA.tar.gz file: FAILED"
fi 
#
###########################################################
# GALAXY LOC FILES
...
#
# fastq_screen.loc
echo "# fastq_screen.loc (colorspace): #"
echo "# Autogenerated .loc file for fastq_screen" > ${TOP_DIR}/fastq_screen.loc
for screen in `ls fastq_screen 2>&1 | grep -v "_nt.conf"` ; do
    echo -n Examining $screen ...
