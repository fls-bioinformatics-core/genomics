...
class QCReporter(object):
    ...
    def __init__(self,dirn,data_format=None,qc_dir='qc',regex_pattern=None,
        ...
        Arguments:
          ...
          version: (optional) version of the QC reporter (default
            is the version of the bcftbx package)
        ...
        # Version
        if version is None:
            self.__version = "bcftbx-%s" % get_version()
        else:
    ...
    def __init_html(self):
        ...
        html = htmlpagewriter.HTMLPageWriter("%s/%s: QC report" % (self.run,self.name))
        # Title
        html.add("<h1>%s/%s: QC report</h1>" % (self.run,self.name))
        # Info on reporter version and date
        html.add("<p>Report generated by qcreporter %s on %s</p>" % (self.__version,
                                                                     time.asctime()))
        # Add CSS rules
            ...
            for s in self.screens():
                ...
                if inline_pngs:
                    pngdata = "data:image/png;base64," + \
                        htmlpagewriter.PNGBase64Encoder().encodePNG(
                            os.path.join(self.qc_dir,s))
                else:
...
class IlluminaQCReporter(QCReporter):
    ...
    def __init__(self,dirn,data_format=None,qc_dir='qc',regex_pattern=None,
        ...
        QCReporter.__init__(self,dirn,data_format=data_format,qc_dir=qc_dir,
                            regex_pattern=regex_pattern,version=version)
...
class SolidQCReporter(QCReporter):
    ...
    def __init__(self,dirn,data_format=None,qc_dir='qc',regex_pattern=None,
        ...
        # Initialise base class
        QCReporter.__init__(self,dirn,data_format=data_format,qc_dir=qc_dir,
                            regex_pattern=regex_pattern,version=None)
