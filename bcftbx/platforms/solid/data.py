...
class Run:
    ...
    def __init__(self, solid_run_dir):
        ...
        if self.run_defn_filn:
            ...
            self.run_definition = RunDefinition(self.run_defn_filn)
            # Get run name and info
            self.run_name = self.run_definition.runName
            self.run_info = RunInfo(self.run_name)
            # Populate libraries
    ...
    def add_library(self, sample_name, library_name, libraries_dir,
        ...
        try:
            ...
            if sample is None:
                # New sample
                sample = Sample(sample_name,parent_run=self)
                ...
                for f in os.listdir(libraries_dir):
                    if f.startswith("BarcodeStatistics"):
                        ...
                        sample.barcode_stats = \
                                    BarcodeStatistics(barcode_stats_filn)
                        break
            else:
            ...
            if os.path.isdir(unassigned_dir):
                ...
                sample.unassigned = Library("unassigned",
...
class Sample:
    ...
    def addLibrary(self, library_name):
        ...
        if not library:
            ...
            library = Library(library_name,parent_sample=self)
